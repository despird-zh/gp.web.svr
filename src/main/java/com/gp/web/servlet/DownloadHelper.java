...
public class DownloadHelper {
	...
	public static void processRequest(HttpServletRequest request, HttpServletResponse response, boolean content)
		...
		List<ContentRange> ranges = new ArrayList<ContentRange>();
		...
		try {
			...
			if (ranges.isEmpty() || ranges.get(0) == full) {
				...
				response.setHeader("Content-Range", "bytes " + r.getStartPos() + "-" + r.getEndPos() + "/" + r.getFileSize());
				...
				if (content) {
					if (acceptsGzip) {
						...
						// browser will hang.
						response.setHeader("Content-Length", String.valueOf(r.getRangeLength()));
					}
				...
				if (content) {
					...
					for (ContentRange r : ranges) {
						...
						sos.println("Content-Range: bytes " + r.getStartPos() + "-" + r.getEndPos() + "/" + r.getFileSize());
